(() => {
  ...
  function scrollInfo(z) {
    ...
    window.setTimeout(() => {
      ...
      const cardTopInsidePanel = panel.scrollTop + (cardRect.top - panelRect.top);
      const centerOffset = Math.max(12, (panel.clientHeight - card.offsetHeight) / 2);
      const maxScroll = Math.max(0, panel.scrollHeight - panel.clientHeight);
      const targetTop = Math.max(0, Math.min(maxScroll, cardTopInsidePanel - centerOffset));

      panel.scrollTo({ top:targetTop, behavior:"smooth" });
      card.classList.add("mw-pulse");
      window.setTimeout(() => card.classList.remove("mw-pulse"), 850);
    }, 180);
  }
  ...
  function select(z, fit=false, scroll=false) {
    ...
    if (fit && R.map) {
      const b=boundsFor(z);
      if (b) {
        try { R.map.setBounds(b,40,40,40,40); }
        catch { try { R.map.setBounds(b); } catch {} }
      }
    }
  ...
  function patchKakao() {
    ...
    function M(...a){
      ...
      try {
        kakao.maps.event.addListener(m,"click",()=>{
          if(Date.now()>=R.ignoreMapUntil) clearSelected();
        });
      } catch {}
      return m;
    }
    ...
    function P(o={}){
      ...
      R.all.push({
        ...
        path:o.path,
        opt:{
          strokeWeight:o.strokeWeight||3,
          strokeColor:o.strokeColor||o.fillColor||"#2563eb",
          strokeOpacity:o.strokeOpacity??1,
          strokeStyle:o.strokeStyle||"solid",
          fillColor:o.fillColor||o.strokeColor||"#2563eb",
          fillOpacity:o.fillOpacity??.24,
          zIndex:o.zIndex||0
        }
      });
      return p;
    ...
    M.prototype=OM.prototype;
    P.prototype=OP.prototype;
    O.prototype=OO.prototype;
    try {
      Object.setPrototypeOf(M,OM);
      Object.setPrototypeOf(P,OP);
      Object.setPrototypeOf(O,OO);
    } catch {}
    K.Map=M;
    K.Polygon=P;
    K.CustomOverlay=O;
  ...
  function waitAndSelect(z, scroll=false) {
    let tries=0;
    const timer=setInterval(()=>{
      tries++;
      if ((R.byZip.get(z)||[]).length) {
        clearInterval(timer);
        select(z,true,scroll);
      } else if (tries>=30) {
        clearInterval(timer);
      }
    },100);
  ...
  function activateZip(z, scroll=false) {
    z=zip(z); if(!z) return;
    if((R.byZip.get(z)||[]).length) {
      select(z,true,scroll);
      return;
  ...
  function chipEnhance() {
    document.querySelectorAll("#zipList .chip").forEach(el=>{
      ...
      el.title=`${z} 폴리곤 선택`;
      if(el.dataset.mwBound)return;
      el.dataset.mwBound="1";
  ...
  function cardEnhance() {
    document.querySelectorAll("#terrainList [data-terrain-zip]").forEach(card=>{
      const z=zip(card.dataset.terrainZip); if(!z)return;
      card.tabIndex=0;
      card.setAttribute("role","button");
      card.title=`${z} 폴리곤 선택`;
      if(card.dataset.mwBound)return;
      card.dataset.mwBound="1";
  ...
  function bar(label,value,cls){
    return `<div class="mw-slope-row ${cls}"><span>${label}</span><i><b style="width:${Math.max(0,Math.min(100,num(value)||0))}%"></b></i><strong>${pct(value)}</strong></div>`;
  }

  function cleanCards() {
    document.querySelectorAll("#terrainList [data-terrain-zip]").forEach(card=>{
      const z=zip(card.dataset.terrainZip), t=R.terrain.get(z);
      if(!z||!t)return;
      const s=t.slope;
  ...
  function styles(){
    ...
    s.textContent=`
      #zipList .chip{cursor:pointer;transition:.16s}
      #zipList .chip:hover{border-color:#fca5a5;background:#fff7f7}
      #zipList .chip.mw-selected{border-color:#ef4444;background:#fff1f2;box-shadow:0 0 0 3px rgba(239,68,68,.16)}
      .terrainSourceBadge,.terrainSub{display:none!important}
      .terrainCard{cursor:pointer;transition:.18s}
      .terrainCard:hover{border-color:#fca5a5;background:#fffafa}
      .terrainCard.mw-selected{border-color:#ef4444!important;background:#fff1f2!important;box-shadow:0 0 0 3px rgba(239,68,68,.16)}
      .terrainCard.mw-pulse{animation:mwp .85s ease}
      .terrainCard h4{margin:10px 0 5px;color:#334155;font-size:10px}
      .terrainCard p{margin:4px 0;color:#475569;font-size:11px}
      .terrainCard p strong{color:#0f172a;font-size:12px}
      .mw-slope{display:grid;gap:7px;margin-top:7px}
      .mw-slope-row{display:grid;grid-template-columns:42px 1fr 42px;gap:8px;align-items:center;font-size:10px}
      .mw-slope-row span,.mw-slope-row strong{font-weight:1000}
      .mw-slope-row strong{text-align:right}
      .mw-slope-row i{height:8px;background:#e2e8f0;border-radius:999px;overflow:hidden}
      .mw-slope-row i b{display:block;height:100%;background:#60a5fa}
      .mw-slope-row.gentle i b{background:#f59e0b}
      .mw-slope-row.steep i b{background:#ef4444}
      .mw-wait{color:#94a3b8!important}
      @keyframes mwp{50%{box-shadow:0 0 0 7px rgba(239,68,68,.20)}}`;
  ...
  document.addEventListener("DOMContentLoaded",()=>{
    ...
    ["zipList","terrainList"].forEach(id=>{
      const e=document.getElementById(id);
      if(e)o.observe(e,{childList:true,subtree:true});
    });
