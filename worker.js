...
async function handleCampsBatch(request, env) {
  const body = await readJson(request);
  ...
  const cleanPayload = (input = {}) => {
    ...
    for (const key of fields) {
      ...
      else if (key === "latitude" || key === "longitude") out[key] = input[key] == null || input[key] === "" ? null : Number(input[key]);
      else out[key] = safeTrim(input[key]) || null;
    }
  ...
  for (const item of updates) {
    ...
    const rows = await supabaseFetch(env, `/rest/v1/${CAMPS_TABLE}?id=eq.${id}&select=*`, {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify(cleanPayload(item?.payload || {})),
    });
